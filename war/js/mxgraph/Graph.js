...
HoverIcons.prototype.init = function()
{
    ...
    this.myPerformanceBenchmark = createPerformanceBenchmarkDiv();
    ...
    this.myPerformanceBenchmark.style.visibility='hidden'
 ...
 *
 */
var flagGoal=0,flagResilience=0,flagDisruption=0,flagRequirement=0,flagOthers=0,flagResource=0,flagAgent=0,flagDomainProperty=0,flagTestCase=0,flagText=0,flagPerformance=0,checkNum=1,checkNum2=1;
HoverIcons.prototype.repaint = function()
{
	...
	if (this.currentState != null)
	{
			...
			this.graph.isCellConnectable(this.currentState.cell))
		{
			...
			this.myPerformanceBenchmark.style.left = Math.round(bds.x + bds.width + 3*this.tolerance) + 'px';
			this.myPerformanceBenchmark.style.top = Math.round(this.currentState.getCenterY() - this.triangleRight.height / 2 - this.tolerance) + 'px';
			...
			var mperformancediv = this.myPerformanceBenchmark;
			...
			this.myTableGoal.onclick=function () {
				if(curr.style['shape']=='goal'){
					if (mgoaldiv.style.visibility == 'visible') {
                        ...
                        var ifbr = (getmyWant(gm,that.id,'target','requirement',"RefinedBy")=='')?'':"</br>";
                        that.RefinedBy=getmyWant(gm,that.id,'target','goal',"RefinedBy")+ifbr+getmyWant(gm,that.id,'target','requirement',"RefinedBy");
                        ...
                        document.getElementById("RefinedBy").innerHTML=getmyWant(gm,that.id,'target','goal',"RefinedBy")+ifbr+getmyWant(gm,that.id,'target','requirement',"RefinedBy");
			...
			this.myTableRequirement.onclick=function () {
				if(curr.style['shape']=='requirement'){
					if(mrequirementdiv.style.visibility=='visible'){
						...
						that.RefinesTo=getmyWant(gm,that.id,'source','goal','RefinesToReq');
						that.Agents=getmyWant(gm,that.id,'target','hexagon','Agents');
						flagRequirement=0;
						mrequirementdiv.style.visibility = 'hidden';
                        ...
                        document.getElementById("RefinesToReq").innerHTML=getmyWant(gm,that.id,'source','goal','RefinesToReq');
                        document.getElementById("Agents").innerHTML=getmyWant(gm,that.id,'target','hexagon','Agents');
						mrequirementdiv.style.visibility = 'visible';
					}
				}
			};
...
*/
			this.myTableOthers.onclick=function () {
                if (curr.style['shape'] == 'resource') {
                        if (mrresourcediv.style.visibility == 'visible') {
                            ...
                            that.RelateTo = getmyWant(gm,that.id, "target",'resource',"RelateTo");
                            flagResource = 0;
                            mrresourcediv.style.visibility = 'hidden';
                            ...
                            document.getElementById("RelateTo").innerHTML = getmyWant(gm,that.id, "target",'resource',"RelateTo");
                            mrresourcediv.style.visibility = 'visible';
                        }
                }
                else if(curr.style['shape'] == 'rectangle'){
					...
					if (mresiliencediv.style.visibility == 'visible') {
						...
						that.AlertRule = document.getElementById("AlertRule").value;
						...
						document.getElementById("AlertRule").value = that.AlertRule;
					...
					if (mtestcasediv.style.visibility == 'visible') {
						...
						that.disrNum = getmyWantNum(gm,that.id,'target','disruption');
						var diff = that.resiNum - checkNum;
						var diff2 = that.disrNum - checkNum2;
						if(diff > 0){
							for(var i = that.resiNum-1; i >= that.resiNum-diff; i--){
								...
								document.getElementById("checkpointdetail").appendChild(addGoalAlertRule(i));
							...
							for(var i = checkNum -1; i >=that.resiNum; i--){
								...
								var testGoalAlertRuletemp = "TestGoalAlertRule"+idnumber;
								var testGoalAlertRule = testGoalAlertRuletemp;
								document.getElementById("checkpointdetail").removeChild(document.getElementById(testGoalAlertRule));
						...
						if(diff2 > 0){
							for(var i = that.disrNum-1;i >= that.disrNum-diff2; i--) {
								document.getElementById("actiondetail").appendChild(addDisName(i));
								document.getElementById("actiondetail").appendChild(addDisTarget(i));
								document.getElementById("actiondetail").appendChild(addDisDescription(i));
								checkNum2++;
							}
						}
						else if(that.disrNum > 0){
							for(var i = checkNum2 - 1; i >=that.disrNum; i--){
								var idnumber = that.i * (-1);
								// var idnumber = 1;
								var testDisNametemp = "TestDisName"+idnumber;
								var testDisName = testDisNametemp;
								document.getElementById("actiondetail").removeChild(document.getElementById(testDisName));
								var testDisTargettemp = "TestDisTarget"+idnumber;
								var testDisTarget = testDisTargettemp;
								document.getElementById("actiondetail").removeChild(document.getElementById(testDisTarget));
								var testDisDescriptiontemp = "TestDisDescription"+idnumber;
								var testDisDescription = testDisDescriptiontemp;
								document.getElementById("actiondetail").removeChild(document.getElementById(testDisDescription));
								checkNum2--;
							}
						}
						...
						that.testDisname=getmyWant(gm,that.id,'target','disruption','testDisname');
						that.testDistarget=getmyWantAttri(gm,that.id,'target','disruption','TargetResDis');
						that.testDisdescription=getmyWantAttri(gm,that.id,'target','disruption','description');
						...
						that.testAlertRule = getmyWantAttri(gm,that.id,'target','resilience', 'AlertRule');
						...
						document.getElementById("testDisname").innerHTML = '';
						document.getElementById("testDistarget").innerHTML = '';
						document.getElementById("testDisdescription").innerHTML = '';
						document.getElementById("testGoalname").innerHTML = '';
						document.getElementById("testGoaltarget").innerHTML = '';
						document.getElementById("testGoalalertrule").innerHTML = '';
						...
						if(that.resiNum > 1){
							for(var current = 1; current < that.resiNum; current++){
								...
								var testalertruletemp = "testGoalalertrule"+curren;
								var testalertrule = testalertruletemp;
								document.getElementById(testalertrule).innerHTML = '';
						...
						if(that.disrNum > 1){
							for(var current = 1; current < that.disrNum; current++){
								var curren = current;
								var testNametemp = "testDisname" + curren;
								var testName = testNametemp;
								document.getElementById(testName).innerHTML = '';
								var testTargettemp = "testDistarget"+curren;
								var testTarget = testTargettemp;
								document.getElementById(testTarget).innerHTML = '';
								var testDescriptiontemp = "testDisdescription"+curren;
								var testDescription = testDescriptiontemp;
						...
						document.getElementById("testDisname").innerHTML = getmyWantSingle(gm,that.id,'target','disruption','value', first);
						console.log(document.getElementById("testDisname").innerHTML);
						document.getElementById("testDistarget").innerHTML =  getmyWantSingle(gm,that.id,'target','disruption','TargetResDis', first);
						console.log(document.getElementById("testDistarget").innerHTML);
						document.getElementById("testDisdescription").innerHTML =  getmyWantSingle(gm,that.id,'target','disruption','description', first);
						console.log(document.getElementById("testDisdescription").innerHTML);
						...
						document.getElementById("testGoalalertrule").innerHTML = getmyWantSingle(gm,that.id,'target','resilience','AlertRule', first);
						...
						if(that.resiNum > 1){
							for(var current = 1; current < that.resiNum; current++){
								...
								var testalertruletemp = "testGoalalertrule" + curren;
								var testalertrule = testalertruletemp;
								document.getElementById(testalertrule).innerHTML = getmyWantSingle(gm,that.id,'target','resilience','AlertRule', curren);
						...
						if(that.disrNum > 1){
							for(var current = 1; current < that.disrNum; current++){
								...
								var testNametemp = "testDisname" + curren;
								var testName = testNametemp;
								document.getElementById(testName).innerHTML = getmyWantSingle(gm,that.id,'target','disruption','value', curren);
								var testTargettemp = "testDistarget"+curren;
								var testTarget = testTargettemp;
								document.getElementById(testTarget).innerHTML = getmyWantSingle(gm,that.id,'target','disruption','TargetResDis', curren);

								var testalertruletemp = "testDisdescription" + curren;
								var testalertrule = testalertruletemp;
								document.getElementById(testalertrule).innerHTML = getmyWantSingle(gm,that.id,'target','disruption','description', curren);
                    ...
                    if (motherdiv.style.visibility == 'visible') {
                        ...
                        if ($('#Obstacle').length != 0) {
                            ...
                            var ifbr = (getmyWant(gm,that.id,'target','requirement',"Target")=='')?'':"</br>";
                            that.Goal = getmyWant(gm, that.id, 'target', 'goal', 'Target')+ifbr+getmyWant(gm,that.id,'target','requirement',"Target");
                        }
                        motherdiv.style.visibility = 'hidden';
                        ...
                        if (curr.style['shape'] == 'obstacle') {
                            if ($('#IObstacle').length == 0) {
                                ...
                                var ifbr = (getmyWant(gm,that.id,'target','requirement',"Target")=='')?'':"</br>";
                                document.getElementById('Target').innerHTML = getmyWant(gm, that.id, 'target', 'goal', 'Target')+ifbr+getmyWant(gm,that.id,'target','requirement',"Target");
			...
			if(mgoaldiv.style.visibility=="visible"){
				...
				||document.getElementById("preCondition").value!=that.preCondition
				||document.getElementById("aftCondition").value!=that.aftCondition
				||document.getElementById("RefinesTo").value!=getmyWant(gm,that.id,'source','goal',"RefinesTo")
				||document.getElementById("RefinedBy").value!=getmyWant(gm,that.id,'target','goal',"RefinedBy"))){
                    ...
                    this.currentState.cell.RefinesTo=getmyWant(gm,that.id,'source','goal',"RefinesTo");
                    var ifbr = (getmyWant(gm,this.currentState.cell.id,'target','requirement',"RefinedBy")=='')?'':"</br>";
                    this.currentState.cell.RefinedBy=getmyWant(gm,that.id,'target','goal',"RefinedBy")+ifbr+getmyWant(gm,this.currentState.cell.id,'target','requirement',"RefinedBy");
                    ...
                    document.getElementById("RefinesTo").innerHTML=getmyWant(gm,that.id,'source','goal',"RefinesTo");
                    var ifbr = (getmyWant(gm,this.currentState.cell.id,'target','requirement',"RefinedBy")=='')?'':"</br>";
                    document.getElementById("RefinedBy").innerHTML=getmyWant(gm,that.id,'target','goal',"RefinedBy")+ifbr+getmyWant(gm,this.currentState.cell.id,'target','requirement',"RefinedBy");
			...
			if(mrequirementdiv.style.visibility=="visible"){
				if((document.getElementById("reqId").innerHTML==that.value)&&(document.getElementById("basicEventFlow").value!=that.basicEventFlow
                    ...
                    this.currentState.cell.Agents=getmyWant(gm,that.id,'target','hexagon','Agents');
                    this.currentState.cell.RefinesTo=getmyWant(gm,that.id,'source','goal','RefinesToReq');
                }
				else{
			...
			if(mrresourcediv.style.visibility=="visible"){
                if((document.getElementById("resourId").innerHTML==that.value)&&(document.getElementById("resourDesc").value!=that.basicEventFlow
                    || document.getElementById("RelateTo").value!=getmyWant(gm,that.id,'target','resource',"RelateTo"))){
                    ...
                    this.currentState.cell.RelateTo=getmyWant(gm,that.id,'target','resource',"RelateTo");
			...
			if(motherdiv.style.visibility=="visible"){
                ...
                if(this.currentState.cell.flag!='obstacle'){
                    ...
                    var ifbr = (getmyWant(gm,that.id,'target','requirement',"Target")=='')?'':"</br>";
                    this.currentState.cell.Goal = getmyWant(gm,that.id,'target','goal','Target')+ifbr+getmyWant(gm,that.id,'target','requirement',"Target");
			...
			else
			{
                ...
                yhis.myPerformanceBenchmark.style.visibility='visible';
...
HoverIcons.prototype.setCurrentState = function(state)
{
    ...
    this.graph.container.appendChild(this.myPerformanceBenchmark);
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
        ...
        mxConnectionHandler.prototype.resolveAndConflict = function(a){
            ...
            if((END_GRAPH == "obst" || END_GRAPH == "disr")&& (START_GRAPH == "requirement"||START_GRAPH == "goal"||START_GRAPH == "resilience")) {
        ...
        mxConnectionHandler.prototype.changeEdgeStyle = function(me)
        {
            ...
            for (var key in this.graph.currentEdgeStyle)
            {
                ...
                if(key == 'strokeColor') {
                		if((START_GRAPH == "goal" || START_GRAPH == "resilience")&& (END_GRAPH == "reso" || END_GRAPH =="requ"||END_GRAPH=="test")) {
                            this.graph.currentEdgeStyle[key] = '#000fff';
                            state.style[key] = this.graph.currentEdgeStyle[key];
                        }
                        else if((START_GRAPH == "obstacle"|| START_GRAPH == "disruption") && (END_GRAPH == "reso"||END_GRAPH=="test")){
                            this.graph.currentEdgeStyle[key] = '#cc0000';
                            state.style[key] = this.graph.currentEdgeStyle[key];
                        }
						...
						else if(START_GRAPH == "hexagon" && (END_GRAPH == "requ"||END_GRAPH == "goal" || END_GRAPH == "resilience") ){
                        ...
                        else if(START_GRAPH == "resource" || (START_GRAPH == "resilience" && END_GRAPH == "perf")){
